...
async function seed() {
  ...
  // ── User causes ──────────────────────────────────────────────────────────────
  await conn.query('DELETE FROM user_causes WHERE user_id = 1');
  await conn.query(`
  ...
  // ── Audience geography ───────────────────────────────────────────────────────
  await conn.query('DELETE FROM audience_geography WHERE user_id = 1');
  await conn.query(`
  ...
  // ── SNS accounts ─────────────────────────────────────────────────────────────
  await conn.query('DELETE FROM sns_accounts WHERE user_id = 1');
  await conn.query(`
  ...
  await conn.query(`
    ...
    (1,'Daily Meals for Homeless Families','🍽️','Feed the Future','GoFundMe','Food Relief',
     ...
     'Providing 500+ daily hot meals to homeless families across three US cities. Help us reach the $100K goal on GoFundMe.'),
    (2,'Build a School in Rural Ghana','🏫','Pencils of Promise','Kickstarter','Education',
     75000,54200,350,100,'2026-05-10','open',
     'Pencils of Promise is crowdfunding a new primary school in the Volta Region of Ghana for 320 children.'),
    (3,'Legal Aid for Syrian Refugees','🤝','International Rescue Committee','Kickstarter','Refugees',
     50000,21000,600,0,'2026-05-20','open',
     'Funding legal support services for Syrian refugees navigating immigration proceedings.'),
    (4,'Mobile Clinics for Rural Kenya','💉','Doctors Without Borders','Indiegogo','Healthcare',
     150000,79500,450,0,'2026-06-01','open',
     'Funding mobile medical clinic trucks to reach underserved communities in rural Kenya.'),
    (5,'Emergency Housing for Flood Survivors','🏘️','Habitat for Humanity','GoFundMe','Housing',
     50000,9500,400,0,'2026-05-30','open',
     'Providing emergency shelter and permanent housing solutions for flood-displaced families.'),
    (6,'Clean Water Wells in Bangladesh','🌊','charity: water','Kickstarter','Clean Water',
     60000,0,350,0,'2026-06-15','open',
     'Drilling and maintaining clean water wells in rural Bangladesh communities.'),
    ...
    (12,'Literacy First','📚','Room to Read','Kickstarter','Education',
     ...
     'Completed literacy program reaching 1,500 children across four schools.')
    ON DUPLICATE KEY UPDATE raised_amount=VALUES(raised_amount), status=VALUES(status)
  `);

  // ── Deliverable templates ─────────────────────────────────────────────────────
  await conn.query('DELETE FROM project_deliverable_templates WHERE project_id IN (1,2)');
  await conn.query(`
  ...
  await conn.query(`
    INSERT INTO user_projects (id, user_id, project_id, enrolled_at, status) VALUES
    (1,1,1,'2026-04-01','active'),
    (2,1,2,'2026-04-15','active'),
    (3,1,7,'2026-02-01','completed'),
    (4,1,8,'2026-03-01','completed'),
    (5,1,9,'2026-03-15','completed'),
    (6,1,10,'2026-01-01','completed'),
    (7,1,11,'2026-02-01','completed'),
    (8,1,12,'2026-01-01','completed')
    ON DUPLICATE KEY UPDATE status=VALUES(status)
  `);

  // Update completed_at for completed projects
  await conn.query(`UPDATE user_projects SET completed_at='2026-02-28' WHERE id=3`);
  await conn.query(`UPDATE user_projects SET completed_at='2026-03-15' WHERE id=4`);
  await conn.query(`UPDATE user_projects SET completed_at='2026-03-31' WHERE id=5`);
  await conn.query(`UPDATE user_projects SET completed_at='2026-01-31' WHERE id=6`);
  await conn.query(`UPDATE user_projects SET completed_at='2026-02-20' WHERE id=7`);
  await conn.query(`UPDATE user_projects SET completed_at='2026-01-30' WHERE id=8`);

  // ── Deliverables ──────────────────────────────────────────────────────────────
  await conn.query('DELETE FROM deliverables WHERE user_project_id IN (1,2)');
  await conn.query(`
    INSERT INTO deliverables (user_project_id, content_type, platform, icon, status, feedback, submitted_at, reviewed_at) VALUES
    (1,'Instagram Reel','instagram','📸','approved','Powerful story ✓','2026-04-15','2026-04-16'),
    (1,'YouTube Documentary','youtube','▶️','approved',NULL,'2026-04-15','2026-04-16'),
    (1,'Instagram Story Set','instagram','📱','revision','Missing GoFundMe link in swipe-up','2026-04-16',NULL),
  ...
  // ── Offers ────────────────────────────────────────────────────────────────────
  await conn.query('DELETE FROM offers WHERE user_id = 1');
  await conn.query(`
  ...
  // ── Materials ─────────────────────────────────────────────────────────────────
  await conn.query('DELETE FROM materials WHERE project_id IN (1,2)');
  await conn.query(`
  ...
  // ── Analytics ─────────────────────────────────────────────────────────────────
  await conn.query('DELETE FROM analytics WHERE user_id = 1');
  await conn.query(`
  ...
  // ── Monthly analytics ─────────────────────────────────────────────────────────
  await conn.query('DELETE FROM analytics_monthly WHERE user_id = 1');
  await conn.query(`
  ...
  // ── Transactions ──────────────────────────────────────────────────────────────
  await conn.query('DELETE FROM transactions WHERE user_id = 1');
  await conn.query(`
