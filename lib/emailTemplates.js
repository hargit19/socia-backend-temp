...
function npoCreatorApplied(npoName, influencerName, projectTitle) {
  return {
    subject: `${influencerName} wants to join "${projectTitle}"`,
    html: wrap('New creator application 🙋', [
      p(`Hi ${npoName}, <strong>${influencerName}</strong> has applied to promote <strong>${projectTitle}</strong>.`),
      p('Review their profile and approve or reject the application from your NPO dashboard, under "Influencer Approvals". A Socia admin will also need to sign off before they go live.'),
    ].join('')),
  };
}
...
function adminApprovalNeeded(kind, label, meta) {
  const KIND_LABEL = { npo: 'NPO registration', project: 'project submission', influencer: 'creator registration', creator_application: 'creator campaign application' };
  return {
